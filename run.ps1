<#
.SYNOPSIS
  DuckNet L7 Security 起動ランチャ (Windows / PowerShell 5+ ・ pwsh)
.DESCRIPTION
  Python を堅牢に検出し、バージョンを確認し、UTF-8 を整えて製品本体へ委譲する。
    .\run.ps1                      # ゲートウェイ(前衛 + 管理ダッシュボード)
    .\run.ps1 --help               # 製品の全オプション
  環境変数:
    DUCKNET_PYTHON    使う Python を明示(未設定なら .venv -> python -> py)
...
function Die([string] $msg) { [Console]::Error.WriteLine("DuckNet: $msg"); exit 9 }

# 1) 任意の設定ファイル(KEY=VALUE / # はコメント)
$envFile = if ($env:DUCKNET_ENV_FILE) { $env:DUCKNET_ENV_FILE } else { Join-Path $self 'app.env' }
...
function Find-Python {
  if ($env:DUCKNET_PYTHON) {
    $fp = $env:DUCKNET_PYTHON
...
$py = Find-Python
if (-not $py) { Die 'Python が見つかりません。Python 3.10+ を導入するか DUCKNET_PYTHON を設定してください。' }
