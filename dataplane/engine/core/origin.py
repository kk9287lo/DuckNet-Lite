...
バックエンドの IP を直叩きして *DuckNet を迂回* する。本モジュールはそれを構造的に潰す。

  · エッジ(DuckNet)は転送するリクエストに、共有鍵から導く *時間バケット HMAC* トークンを付与する。
...
(env `DUCKNET_ORIGIN_KEY`)。鍵を VM 外(KMS/シークレット管理)に置けば、スナップショット窃取
されてもトークンは偽造できない。
"""
