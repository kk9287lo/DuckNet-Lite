...
> 正直に先に。**DuckNet の「終了されにくさ・改竄されにくさ」は OS が公式に用意する保護機構で
...
```
鍵ファイルは `root:ducknet` 所有・`0640`、または Vault/KMS 等から起動時に注入する。
...
### 1.3 最小権限で動かす
専用の非特権ユーザ（例 `ducknet`）で実行し、書込みは state dir だけに限定する。バインドに特権
